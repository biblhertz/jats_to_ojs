...
class OJSNativeAdapter {
     ...
     */
    function writeCover() {
        if (trim($this->article->getCoverImageFilePath() == "")) return;
        ...
        $this->xmlWriter->writeRaw($this->article->getCoverImageFileName());
        $this->xmlWriter->endElement();

        $this->xmlWriter->startElement("cover_image_alt_text");
        $this->xmlWriter->writeRaw($this->article->getCoverImageAltText());
        $this->xmlWriter->endElement();

        $this->xmlWriter->startElement("embed");
        $this->xmlWriter->writeAttribute("encoding", "base64");
        $this->xmlWriter->writeRaw($this->article->getCoverImageAsBase64());
        $this->xmlWriter->endElement();
   ...
   private  function writeSubmissionFile() {

        if (trim($this->article->getGalleyFilePath() == "")) return;

        $this->xmlWriter->startElement("submission_file");
        $this->xmlWriter->writeAttribute("xmlns:xsi","http://www.w3.org/2001/XMLSchema-instance");
        $this->xmlWriter->writeAttribute("id", 100);
		$this->xmlWriter->writeAttribute("file_id", 100);
        ...
        $this->addLocaleAttribute();
        $this->xmlWriter->writeRaw($this->article->getOJSUserName() . ", " . $this->article->getGalleyFileName());
        $this->xmlWriter->endElement();

        $this->xmlWriter->startElement("file");
		$this->xmlWriter->writeAttribute("id", 100);
		$this->xmlWriter->writeAttribute("filesize", $this->article->getGalleyFileSize());
		$this->xmlWriter->writeAttribute("extension", $this->article->getGalleyFileType());

        $this->xmlWriter->startElement("embed");
        $this->xmlWriter->writeAttribute("encoding", "base64");
        $this->xmlWriter->writeRaw($this->article->getGalleyFileAsBase64());
        $this->xmlWriter->endElement();
    ...
    function writeArticleGalley() {
        ...
        $this->xmlWriter->startElement("name");
        $this->addLocaleAttribute();
        $this->xmlWriter->writeRaw(strtoupper($this->article->getGalleyFileType()));
        $this->xmlWriter->endElement();
        ...
        // Disabled for OJS 3.2
//        $this->xmlWriter->startElement("remote");
//        $this->xmlWriter->writeAttribute("src", $pdfUrl . xmlFormat($fileName));
//        $this->xmlWriter->endElement();
